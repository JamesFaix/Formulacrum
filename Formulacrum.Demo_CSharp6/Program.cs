...
namespace Formulacrum.Demo {
    ...
    class Program {
        ...
        private static void Write(string label, object obj, bool indent = true) {
        ...
        private static void CreateReference() {
            ...
            //These examples are using the A1References module, which uses A1 notation,
            //the R1C1References module uses R1C1 notation.
        ...
        private static void OutlineRendering() {
            ...
            var cols = Enumerable.Range(1, 7)
                .Select(x => Column(Fibonacci(x * 2)))
                .ToArray();

            var n = Formula(NotEqual(
                Average().SetValues(cols), 
                Count().SetValues(cols)));
            ...
            Write("Formula(NotEqual(Average().Setvalues(cols), Count().SetValues(cols)))", n, false);
            Write("Formula(NotEqual(Average().Setvalues(cols), Count().Setvalues(cols))).Render(false)", n.Render(false), false);
            Write("Formula(NotEqual(Average().Setvalues(cols), Count().Setvalues(cols))).Render(true)", n.Render(true),false);
        }
